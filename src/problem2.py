...
def problem2a(circle, rectangle, window):
    ...
    rectur = rectangle.get_upper_right_corner()
    rectll = rectangle.get_lower_left_corner()
    line1 = rg.Line(rectur, rectll)
    line1.arrow = 'last'
...
def problem2b(rect, n, delta, win):
    ...
    for k in range(n-1):
        ...
        width = width + width/2
        rect = rg.Rectangle(rg.Point(center.x - width/2, center.y - height/2), rg.Point(center.x + width/2, center.y + height/2))
        rect.attach_to(win)
    win.render()
